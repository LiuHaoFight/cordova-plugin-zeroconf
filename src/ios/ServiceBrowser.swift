...
class ServiceBrowser: NSObject, NetServiceBrowserDelegate, NetServiceDelegate {
    ...
    func netServiceBrowser(_ browser: NetServiceBrowser, didFind service: NetService, moreComing: Bool) {
        if (service.type == "_tcp.local." || service.type == "_udp.local.") {
            ...
            print("Found type \"\(service.name)\" \"\(service.domain)\"")
            if let found = browsers[service] {
                print("stopping existing browser (shouldn't really happen)")
                found.stop();
            }
            ...
            print("Found service " + service.type)
    ...
    func netServiceBrowser(_ browser: NetServiceBrowser, didRemove service: NetService, moreComing: Bool) {
        if (service.type == "_tcp.local." || service.type == "_udp.local.") {
            if let b = browsers[service] {
                ...
                print("removed type " + service.name)
            } else {
                print("can't remove type " + service.name)
            }
        } else {
            if (services.contains(service)) {
                print("removed service " + service.type)
                services.remove(at: services.index(of: service)!)
                broadcast()
            } else {
                print("can't remove service \(service.type)")
            }
        }
    }
    ...
    func netServiceBrowser(_ browser: NetServiceBrowser, didRemoveDomain domainString: String, moreComing: Bool) {
        print("lost domain \(domainString)")
    }
